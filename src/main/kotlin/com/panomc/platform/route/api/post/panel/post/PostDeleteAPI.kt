...
class PostDeleteAPI : Api() {
    ...
    private fun deletePost(context: RoutingContext, handler: (result: Result) -> Unit) {
    ...
    private fun isPostExistsByID(
        ...
        databaseManager.getSQLConnection(connection).queryWithParams(query, JsonArray().add(id)) { queryResult ->
            if (queryResult.succeeded())
                ...
                databaseManager.closeConnection(connection) {
                    resultHandler.invoke(Error(ErrorCode.DELETE_POST_API_SORRY_AN_ERROR_OCCURRED_ERROR_CODE_57))
                }
        }
    }
    ...
    private fun deletePostByID(
        ...
        databaseManager.getSQLConnection(connection).updateWithParams(query, JsonArray().add(id)) { queryResult ->
            if (queryResult.succeeded())
                ...
                databaseManager.closeConnection(connection) {
                    resultHandler.invoke(Error(ErrorCode.DELETE_POST_API_SORRY_AN_ERROR_OCCURRED_ERROR_CODE_56))
                }
        }
    }
}