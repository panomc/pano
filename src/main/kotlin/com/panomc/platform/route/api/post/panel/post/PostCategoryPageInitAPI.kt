...
class PostCategoryPageInitAPI : Api() {
    ...
    override fun getHandler() = Handler<RoutingContext> { context ->
        ...
        auth.isAdmin(context) { isAdmin ->
            if (isAdmin) {
                ...
                getPostCategoriesPageData(context) { result ->
    ...
    private fun getPostCategoriesPageData(context: RoutingContext, handler: (result: Result) -> Unit) {
    ...
    private fun getPostCategoriesCount(
        ...
        databaseManager.getSQLConnection(connection).queryWithParams(query, JsonArray()) { queryResult ->
            if (queryResult.succeeded())
                ...
                databaseManager.closeConnection(connection) {
                    resultHandler.invoke(Error(ErrorCode.POST_CATEGORY_PAGE_INIT_API_SORRY_AN_ERROR_OCCURRED_ERROR_CODE_42))
                }
        }
    }
    ...
    private fun getCategoriesByPage(
        ...
        databaseManager.getSQLConnection(connection).queryWithParams(query, JsonArray()) { queryResult ->
            if (queryResult.succeeded()) {
                ...
                databaseManager.closeConnection(connection) {
                    resultHandler.invoke(Error(ErrorCode.POST_CATEGORY_PAGE_INIT_API_SORRY_AN_ERROR_OCCURRED_ERROR_CODE_41))
                }
        }
    }
}