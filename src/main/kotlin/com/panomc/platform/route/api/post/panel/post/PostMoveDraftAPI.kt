...
class PostMoveDraftAPI : Api() {
    ...
    private fun moveDraft(context: RoutingContext, handler: (result: Result) -> Unit) {
            ...
            if (connection == null)
                ...
                isPostExistsByID(connection, id, handler) { exists ->
                    if (!exists)
                        ...
                        moveDraftInDB(connection, id, handler) {
                            databaseManager.closeConnection(connection) {
    ...
    private fun isPostExistsByID(
        ...
        databaseManager.getSQLConnection(connection).queryWithParams(query, JsonArray().add(id)) { queryResult ->
            if (queryResult.succeeded())
                ...
                databaseManager.closeConnection(connection) {
                    resultHandler.invoke(Error(ErrorCode.MOVE_DRAFT_POST_API_SORRY_AN_ERROR_OCCURRED_ERROR_CODE_109))
                }
        }
    }

    private fun moveDraftInDB(
        connection: Connection,
        id: Int,
        resultHandler: (result: Result) -> Unit,
        handler: () -> Unit
        ...
        databaseManager.getSQLConnection(connection).updateWithParams(
            ...
            if (queryResult.succeeded())
                ...
                databaseManager.closeConnection(connection) {
                    resultHandler.invoke(Error(ErrorCode.MOVE_DRAFT_POST_API_SORRY_AN_ERROR_OCCURRED_ERROR_CODE_108))
                }
        }
    }
}